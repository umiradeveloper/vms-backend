...
public class ProyekRes {
    ...
    public Response getProyekDashboard() {
        ...
        for (ProyekEntity proE : proyek) {
            ...
            responseProyek.add(new ResponseProyekDto(total_bk, total_pu, currMos, kerja_tambah_total,
                    kerja_kurang_total, total_scurve, proE));
        }
    ...
    public Response getProyekByIdBkPu(
        ...
        try {
            ...
            // ResponseProyekDto res = new ResponseProyekDto(total_bk, total_pu, currMos,
            // kerja_tambah_total, kerja_kurang_total, proyek);
            ResponseProyekDto<ProyekEntity> dto = new ResponseProyekDto<>(total_bk, total_pu, currMos,
                    kerja_tambah_total, kerja_kurang_total, total_scurve, proyek);
    ...
    public Response getBkPuByWeek(
        ...
        try {
            ...
            if(pe != null){
                if(pe.periode_awal_progress != null && pe.periode_akhir_progress != null){
    ...
    public Response getBkKategori(@QueryParam("id_proyek") String id_proyek) {
        ...
        for (KategoriEntity ktgr : kategori) {
            List<BiayaKontruksiEntity> biayaK = BiayaKontruksiEntity.find(
                    "SELECT b FROM BiayaKontruksiEntity b JOIN b.rapa r JOIN r.costCodeRapa c JOIN c.kategori k WHERE b.proyek = ?1 AND c.kategori = ?2",
                    proyek, ktgr).list();
        ...
        // List<BkKategoriDto> bk = BiayaKontruksiEntity.find("SELECT k.kode_kategori as
        // kode_kategori, k.nama_kategori as nama_kategori, SUM(b.harga_total) as biaya
        // FROM BiayaKontruksiEntity b JOIN b.rapa r JOIN r.costCodeRapa c JOIN
        // c.kategori k WHERE b.proyek = ?1 GROUP BY k.nama_kategori, k.kode_kategori",
        // proyek).project(BkKategoriDto.class).list();
    ...
    public record WeekData(int week, LocalDate startDate, LocalDate endDate, BigInteger bk, BigInteger pu,
            BigInteger mos, BigInteger kumulativeBk, BigInteger kumulativePu) {
    }
